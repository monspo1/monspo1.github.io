 ...
 * ------------------------------------------------------------------------------------ */
//http://caseyjhol.github.io/bootstrap-select/
...
$(document).ready(function() {
	...
	$('.selectpicker').selectpicker();                     // selectPicker for Ethnicity
	$('.selectPickerForSideEffects').selectpicker();       // selectPicker for Side Effect
    $('.selectPickerForDiagnosisStage').selectpicker();    // selectPicker for Diagnosis stage
	...
	$('#modalForPatientInfo').click(function() {
		isModalEditing = true;
		//$('#inputForPatientAgeInModal').val(localStorage.ageOfThePatient);
	});
									
	$('#inputForPatientAgeInModal').val(localStorage.ageOfThePatient);
    //$('#selectpickerForDiagnosisStagaeInModal').val(localStorage.selectPickerValueForDiagnosisStage);
});


$(document).bind('click', function(){
   //console.log("*** clicked at " + new Date()); 
    console.log("*** clicked at " + getTimeStamp()); 
...
function gotoDSBtnClick(){
    ...
    if( (!$('#inputForPatientAge').val()) || ($('#inputForPatientAge').val() == "undefined") 
            || ($('#selectPickerForEthnicity').selectpicker('val') == "Select ethnicity") 
            || ($('#selectPickerForDiagnosisStage').selectpicker('val') == "Select Stage") ) 
    {
	   
        var htmlForAlert = '<b>NOTE...</b> Please input the following information ';
        ...
        if( (!$('#inputForPatientAge').val()) || ($('#inputForPatientAge').val() == "undefined")){
                ...
                htmlForAlert = htmlForAlert + '<br/>- Age';
                
        } 
        
        //$('#alertForPatientInfo').is(':visible');;
        if( $('#selectPickerForEthnicity').selectpicker('val') == "Select ethnicity"){
            $('#alertForPatientInfo').show();
            //$('#alertForPatientInfo').html("<b>NOTE...</b> Please choose the patient's ethnicity");
            htmlForAlert = htmlForAlert + '<br/>- Ethnicity';
        } 
        
        if($('#selectPickerForDiagnosisStage').selectpicker('val') == "Select Stage"){
            $('#alertForPatientInfo').show();
            htmlForAlert = htmlForAlert + '<br/>- Diagnosis Stage';          
             
        }
        
        $('#alertForPatientInfo').html(htmlForAlert);
        ...
        if(typeof(Storage)!== "undefined"){
            ...
            localStorage.selectPickerValueForDiagnosisStage = $('#selectPickerForDiagnosisStage').selectpicker('val');
...
function modifyPatientInfoInModal(){
    ...
    if(($('#selectPickerForSideEffects').selectpicker('val') != '') 
        || ($('#selectPickerForSideEffects').selectpicker('val') != 'Nothing selected')){
        //http://silviomoreto.github.io/bootstrap-select/
        $('#selectPickerForSideEffects').selectpicker('val', '');
        
    }
        
    if(hasMedian == true){
        hasMedian = false;
        $('#buttonForMedian').text("Show Medians");
    }
	//$('#inputForPatientAgeInModal').val(localStorage.ageOfThePatient);
	localStorage.ageOfThePatient = $('#inputForPatientAgeInModal').val();
	//localStorage.selectPickerValueForDiagnosisStage = $('#selectpickerForDiagnosisStagaeInModal').val();
    ...
    if(chart.series.length > 8){
        if(typeof(chart.series[8].name)!== 'undefined'){
            if(chart.series[8].name == "SideEffect"){	
    ...
    localStorage.selectPickerValueForEthnicity = $('#selectPickerForEthnicityInModal').val();
    localStorage.selectPickerValueForDiagnosisStage = $('#selectpickerForDiagnosisStagaeInModal').val();
	...
	$("#diagnosisStageInDS").html('<b>Stage at Diagnosis:  </b> Stage ' + localStorage.selectPickerValueForDiagnosisStage);
    ...
    switch(localStorage.selectPickerValueForDiagnosisStage){
        case "I":
            $('#inlineCheckbox2').prop('checked', true);
            break;
        case "II":
...
function updateVisButnClick(){
		...
		console.log("2a. parsedArray.length (in updateVisButnClick): " + parsedArray.length);
		...
		console.log("2b. parsedArray.length: " + parsedArray.length);
...
function drawChartWithSeriesData(){
	...
	*/
	...
	if(currentSideEffectSelected == ""){
		chart.series[0].color = 'rgba(223, 83, 83, .6)';
		chart.series[1].color = 'rgba(119, 152, 191, .6)';
		chart.series[2].color = 'rgba(50, 205, 50, .6)';
		chart.series[3].color = 'rgba(245, 146, 17, .7)';
		chart.series[4].color = 'rgba(199, 21, 133, .6)';
		chart.series[5].color = 'rgba(17, 101, 245, .5)';
		chart.series[6].color = 'rgba(148, 0, 211, .6)'	;
	...
	//*
		...
		chart.series[0].color = 'rgba(181, 181, 181, .6)';
		chart.series[1].color = 'rgba(181, 181, 181, .6)';
		chart.series[2].color = 'rgba(181, 181, 181, .6)';
		chart.series[3].color = 'rgba(181, 181, 181, .6)';
		chart.series[4].color = 'rgba(181, 181, 181, .6)';
		chart.series[5].color = 'rgba(181, 181, 181, .6)';
		chart.series[6].color = 'rgba(181, 181, 181, .6)';
		...
		chart.addSeries({
			...
			tooltip: {
				...
				*/
				...
				headerFormat: '<table>',
				pointFormat: '<tr><td></td><td style="color:{series.color};padding:0">'
				+'</b><br/>'+'<b>Status</b>: </td>' + '<td style="padding:0">{point.isAlive}</td></tr>'
				+'</b><br/>'+'<b>Survival Months</b>: </td>' + '<td style="padding:0">{point.y}</td></tr>'
				+'</b><br/>'+'<b>Stage at diagnosis</b>: </td>' + '<td style="padding:0">{point.dxGrade}</td></tr>'
...
function getMedian(values) {
    ...
    if(values.length == 0){
        ...
        for(var i = 0; i < values.length; i++){
            if(typeof(values[i].survivalMonths) !== 'undefined'){
                sum += parseInt(values[i].survivalMonths);     
            }
        }
...
function showMedians(){
	...
	if(!hasMedian){
		...
		*/
		...
		//$('#buttonForMedian').val('Hide Median');
		$('#buttonForMedian').text("Hide Median");
		//document.getElementById('buttonForMedian').value = "Hide Medians";
...
function decisionspaceOnLoad(){
	...
	console.log("Picker for selectPickerForDiagnosisStage: " + localStorage.selectPickerValueForDiagnosisStage);
    ...
    if( (localStorage.ageOfThePatient > 0) && (localStorage.ageOfThePatient < 10) ){
        minAge = 0;
        maxAge = 10;
    } else if((localStorage.ageOfThePatient >= 10) && (localStorage.ageOfThePatient < 20) ){
        minAge = 10;
        maxAge = 20;
    } else if((localStorage.ageOfThePatient >= 20) && (localStorage.ageOfThePatient < 30) ){
        minAge = 20;
        maxAge = 30;
    } else if((localStorage.ageOfThePatient >= 30) && (localStorage.ageOfThePatient < 40) ){
        minAge = 30;
        maxAge = 40;
    } else if((localStorage.ageOfThePatient >= 40) && (localStorage.ageOfThePatient < 50) ){
        minAge = 40;
        maxAge = 50;
    } else if((localStorage.ageOfThePatient >= 50) && (localStorage.ageOfThePatient < 60) ){
        minAge = 50;
        maxAge = 60;
    } else if((localStorage.ageOfThePatient >= 60) && (localStorage.ageOfThePatient < 70) ){
        minAge = 60;
        maxAge = 70;
    } else if((localStorage.ageOfThePatient >= 70) && (localStorage.ageOfThePatient < 80) ){
        minAge = 70;
        maxAge = 80;
    } else if((localStorage.ageOfThePatient >= 80) && (localStorage.ageOfThePatient < 90) ){
        minAge = 80;
        maxAge = 90;
    } else {
        minAge = 90;
    ...
    $('#selectPickerForEthnicityInModal').selectpicker('val', localStorage.selectPickerValueForEthnicity);
    $('#selectpickerForDiagnosisStagaeInModal').selectpicker('val', localStorage.selectPickerValueForDiagnosisStage);
	...
	for(i=0;i<seerdata.length;i++){
		...
		if(dxGrade == 'Grade I'){
			dxGrade = 'I';	
		} else if(dxGrade == 'Grade II'){
			dxGrade = 'II';	
		} else if(dxGrade == 'Grade III'){
			dxGrade = 'III';	
		} else if(dxGrade == 'Grade IV'){
			dxGrade = 'IV';	
		} 
    ...
    parsedArray = parsedArray.filter(function(el){
       return el.dxGrade == localStorage.selectPickerValueForDiagnosisStage; 
    });
	...
	medianForSurgRad = getMedian(filteredForArrForSurgRad);
	medianForNoTreatment = getMedian(filteredForArrForNoTreatment);
	console.log("medianForNoTreatment: " + medianForNoTreatment);
	...
	$('#chart-container').highcharts({
            ...
            xAxis: {
				...
				*/
				...
				categories: ['Chemo Therapy', 'Chemo therapy & Radiation', 'Hormonal Therapy', 'Radiation', 'Surgery', 'Surgery & Radiation', 'None (Active monitoring)'],
                labels: {
                    style: {
                        fontSize: '13px'   
                    }   
            ...
            yAxis: {
                title: {
                    text: 'Survival time (total # of months)',
                    style: {
                        fontSize: '15px'   
                    }
                },
